...
.\installer.ps1 -AvecParolier              # parolier 8B recommandé (+ ~5 Go)
.\installer.ps1 -AvecParolier -Parolier 4b # parolier 4B léger, GPU 8 Go (+ ~2,5 Go)
.\installer.ps1 -AvecParolier -Parolier les-deux   # les deux modèles (+ ~7,5 Go)
```
...
|---|---|---|---|
| 16 Go (ex. RTX 2000 Ada) | YuE2 **Q8** (~9 Go) + Parolier **8B** (~6 Go) | ~15 Go | ✅ cohabitation directe |
| 12 Go | YuE2 **Q4** (~8 Go) + Parolier **8B** (~6 Go) | ~14 Go | ⚠️ limite : préférez le **4B** (~3 Go), ou libérez la VRAM avant d'écrire |
...
| | Parolier 8B ⭐ | Parolier 4B |
|---|---|---|
| Modèle source | [huihui-ai/Huihui-Qwen3-8B-abliterated-v2](https://huggingface.co/huihui-ai/Huihui-Qwen3-8B-abliterated-v2) | [huihui-ai/Huihui-Qwen3-4B-Instruct-2507-abliterated](https://huggingface.co/huihui-ai/Huihui-Qwen3-4B-Instruct-2507-abliterated) |
| Fichier GGUF | [mradermacher/…v2-GGUF](https://huggingface.co/mradermacher/Huihui-Qwen3-8B-abliterated-v2-GGUF) — `Q4_K_M` | [mradermacher/…-GGUF](https://huggingface.co/mradermacher/Huihui-Qwen3-4B-Instruct-2507-abliterated-GGUF) — `Q4_K_M` |
| Poids | 4,7 Gio | 2,3 Gio |
| VRAM (Q4 + contexte 8k) | ~6 Go | ~3 Go |
| Licence | **Apache 2.0** ✅ | **Apache 2.0** ✅ |

**Pourquoi Qwen3 abliterated ?** Qwen3 est la meilleure base ouverte 2025-2026 pour
le **français** dans cette taille ; la version abliterated de huihui-ai (l'auteur
de référence des abliterations propres, 320/320 au test de non-refus) garde toute
la qualité d'instruction — indispensable pour respecter un format contraint
(TITRE / STYLE / PAROLES, refrain dupliqué, syllabes). La quantification Q4_K_M
de mradermacher (quantificateur communautaire de référence) est le meilleur
compromis taille/qualité. Et la licence Apache 2.0 autorise l'usage commercial
des paroles produites — contrairement aux poids YuE2 (CC BY-NC, voir § 7).
...
| Séries *Heretic* DavidAU / DreamFast (Qwen3-8B-Heretic…) | Souvent excellentes, mais expérimentales et changeantes : moins stables sur un format strict à respecter à la lettre. |
...
| Qwen3.5-9B « HauhauCS » et dérivés 2026 | Très récents et prometteurs, mais peu testés avec llama.cpp et auteur controversé : à réévaluer dans quelques mois. |
...
- **Appel** : `POST /v1/chat/completions` (API compatible OpenAI),
  `temperature 1.0`, `top_p 0.95`, `max_tokens 2048`, graine aléatoire à chaque fois.
  Pas de pénalité de répétition (elle casserait la duplication exigée du refrain).
- **Qwen3 est hybride** (mode raisonnement + mode direct) : YueStudio ajoute
  `/no_think` pour obtenir directement les paroles, et filtre les éventuels blocs
  `<think>…</think>` résiduels.
...
| Méthode | Route | Rôle |
|---|---|---|
| POST | `/api/lyrics` | écrit une chanson (`prompt` complet, ou `style` + `sujet` + `duration` + `lang`) ; renvoie `title`, `style`, `lyrics`, `duration_estimate`, `raw`, `model`, `seed`, `gen_seconds` |
...
| Le serveur LLM s'arrête aussitôt | VRAM insuffisante : 🧹 *Libérer la VRAM*, puis modèle **4B** (`-Parolier 4b`). Détails dans `engine/journal-parolier.log`. |
...
Les modèles paroliers sont sous **Apache 2.0** : les paroles qu'ils écrivent vous
appartiennent, y compris pour un usage commercial. ⚠️ Mais attention : dès qu'elles
sont chantées par YuE2, le **morceau** relève de la licence des poids YuE2
(**CC BY-NC 4.0 — usage non commercial**). Les paroles seules restent libres.
